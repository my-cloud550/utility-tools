            ...
            return (
                <div className="flex h-screen bg-slate-50">
                    ...
                    <main className="flex-1 flex flex-col h-full relative overflow-hidden">
                        <header className="md:hidden bg-white/80 backdrop-blur-md border-b border-slate-200 p-4 flex justify-between items-center sticky top-0 z-20"><span className="font-bold text-lg flex items-center gap-2"><Icon name="box" size={20} className="text-blue-600"/> {t.title}</span><div className="flex gap-2"><button onClick={() => { window.location.href = `/${lang}/`; }} className="p-2 bg-slate-100 rounded-lg text-slate-600"><Icon name="home" size={20} /></button><button onClick={() => { window.location.href = makePath(lang === 'ko' ? 'en' : 'ko', activeToolId); }} className="p-2 bg-slate-100 rounded-lg text-slate-600"><Icon name="globe" size={20} /></button><button onClick={toggleTheme} className="p-2 bg-slate-100 rounded-lg text-slate-600" aria-label="toggle theme"><Icon name={theme === 'dark' ? 'sun' : 'moon'} size={20} /></button><button onClick={() => setMenuOpen(true)} className="p-2 bg-slate-100 rounded-lg text-slate-600"><Icon name="menu" size={20} /></button></div></header>
