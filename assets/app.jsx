...
function getActiveToolIdFromLocation() {
  ...
  if (parts[0] === 'tools' && parts[1]) {
    return ID_BY_SLUG[parts[1]] || 'text';
  }
  // Backward-compat: allow old hashes like #text
  const h = (window.location.hash || '').replace('#','');
  if (h && PATH_BY_ID[h]) return h;
  return 'text';
}
;
...
// Update <title> + meta description for each tool & language
useEffect(() => {
                                ...
                                <div key={cat}>
                                    ...
                                    <div className="space-y-1">
                                        {tools.filter(x => x.cat === cat).map(item => (
                                            <a key={item.id} href={PATH_BY_ID[item.id] || '/'} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${activeTool.id === item.id ? 'bg-slate-800 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}><Icon name={item.icon} size={18} className={activeTool.id === item.id ? 'text-blue-300' : 'text-slate-400'} />{t.tools[item.id]}</a>
                                        ))}
                                    </div>
                                </div>
                            ))}
