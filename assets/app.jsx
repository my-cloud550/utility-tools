...
// ===== Router helpers (pathname-first, hash fallback) =====
const slugToTool = {
  "word-counter": "text",
  "case-converter": "case",
  "percent-calculator": "percent",
};
const toolToSlug = Object.fromEntries(Object.entries(slugToTool).map(([k,v]) => [v,k]));
...
function getToolFromLocation() {
  ...
  if (m && slugToTool[m[1]]) return slugToTool[m[1]];
  const hash = (window.location.hash || "").replace("#", "");
  if (hash && ["text","case","percent","bmi","discount","unit","image","color","stopwatch","pomodoro","dday"].includes(hash)) return hash;
  return "text";
}

function getPathForTool(toolId) {
  const slug = toolToSlug[toolId];
  if (!slug) return "/#" + toolId; // fallback for tools not yet split into pages
