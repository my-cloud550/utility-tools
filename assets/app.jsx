        ...
        // --- Main App & Routing ---
        const App = () => {
            const [lang, setLang] = useState(() => getLangFromPathname() || 'ko');
...
const PATH_BY_ID = {
  ...
  "lotto": "/tools/lotto-generator/"
};


const SUPPORTED_LANGS = ['ko','en'];

function getLangFromPathname(pathname = window.location.pathname) {
...
function stripLangPrefix(pathname = window.location.pathname) {
  ...
  let stripped = String(pathname).replace(new RegExp(`^/${lang}`), '');
...
function withLangPrefix(path, lang) {
  const clean = path || '/';
  if (!SUPPORTED_LANGS.includes(lang)) return clean;
  if (clean === '/') return `/${lang}/`;
  // avoid double-prefix
  if (clean.startsWith(`/${lang}/`) || clean === `/${lang}`) return clean.endsWith('/') ? clean : `${clean}/`;
  const merged = `/${lang}${clean}`.replace(/\/+/g, '/');
...
const ID_BY_SLUG = {
  ...
  "dday-calculator": "dday",
  "password-generator": "password",
  "lotto-generator": "lotto"
};
function getActiveToolIdFromLocation() {
  const raw = (window.location.pathname || '/').replace(/\/+$/, '/');
  const path = stripLangPrefix(raw).replace(/\/+$/, '/');
  const match = path.match(/^\/tools\/([^\/]+)\/?$/);
  if (match) return ID_BY_SLUG[match[1]] || 'text';
  return 'text';
}
;

            const activeToolId = getActiveToolIdFromLocation();
                                ...
                                <div key={cat}>
                                    ...
                                    <div className="space-y-1">
                                        {tools.filter(x => x.cat === cat).map(item => (
                                            <a key={item.id} href={withLangPrefix(PATH_BY_ID[item.id] || '/', lang)} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${activeTool.id === item.id ? 'bg-slate-800 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}><Icon name={item.icon} size={18} className={activeTool.id === item.id ? 'text-blue-300' : 'text-slate-400'} />{t.tools[item.id]}</a>
                                        ))}
                                    </div>
                                </div>
                            ))}
