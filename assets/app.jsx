const { useState, useEffect, useRef, useCallback, useMemo } = React;
        ...
        const App = () => {
            ...
            useEffect(() => {
                const onPop = () => {
                    const r = parseRouteFromPath();
                    setLang(r.lang);
                    setActiveToolId(r.tool);
                };
            ...
            const applyMeta = () => {
                // If this HTML page already ships static SEO (SSG), do not override it at runtime.
                const staticSeoTitle = document.querySelector('title[data-static="1"]');
                if (staticSeoTitle) return;
                const langPack = translations[lang] || translations.ko;
                const seo = (langPack.seo && langPack.seo[activeToolId]) ? langPack.seo[activeToolId] : null;
                const pageTitle = seo && seo.title ? `${seo.title} - ${langPack.title}` : `${langPack.title} - ${langPack.subtitle}`;
                document.title = pageTitle;
                const desc = seo && seo.desc ? seo.desc : (langPack.subtitle || '');
            ...
            const navigateToTool = (e, toolId) => {
                if (e) e.preventDefault();
                setActiveToolId(toolId);
                window.history.pushState({}, '', makePath(lang, toolId));
                setMenuOpen(false);
            };
                                ...
                                <div key={cat}>
                                    ...
                                    <div className="space-y-1">
                                        {tools.filter(x => x.cat === cat).map(item => (
                                            <a key={item.id} href={makePath(lang, item.id)} onClick={(e) => navigateToTool(e, item.id)} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${activeToolId === item.id ? 'bg-slate-800 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}><Icon name={item.icon} size={18} className={activeToolId === item.id ? 'text-blue-300' : 'text-slate-400'} />{t.tools[item.id]}</a>
                                        ))}
                                    </div>
                                </div>
                            ))}
