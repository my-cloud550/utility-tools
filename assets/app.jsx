const { useState, useEffect, useRef, useCallback, useMemo } = React;
    ...
    // ---- Routing helpers (hash + path + per-page tool id) ----
    const TOOL_PATHS = {
    ...
    const pathToToolId = (pathname) => {
        const p = (pathname || '/').replace(/\/+/g, '/');
        if (p.startsWith('/tools/word-counter')) return 'text';
        if (p.startsWith('/tools/case-converter')) return 'case';
        if (p.startsWith('/tools/percent-calculator')) return 'percent';
        return null;
    };

    const toolIdToHref = (toolId) => {
        return TOOL_PATHS[toolId] || `/#${toolId}`;
    };

    const getToolIdFromLocation = () => {
        // Highest priority: page-provided tool id (static pages)
        if (window.__TOOL_ID__) return window.__TOOL_ID__;
        ...
        return 'text';
            ...
            useEffect(() => {
                const toolName = (t.tools && t.tools[activeTool.id]) ? t.tools[activeTool.id] : 'UtilityBox';
                document.title = `${toolName} | UtilityBox`;
            }, [activeTool.id, lang]);
                                ...
                                <div key={cat}>
                                    ...
                                    <div className="space-y-1">
                                        {tools.filter(x => x.cat === cat).map(item => (
                                            <a key={item.id} href={`#${item.id}`} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${hash === item.id ? 'bg-slate-800 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}><Icon name={item.icon} size={18} className={hash === item.id ? 'text-blue-300' : 'text-slate-400'} />{t.tools[item.id]}</a>
                                        ))}
                                    </div>
                                </div>
                            ))}
