const { useState, useEffect, useRef, useCallback, useMemo } = React;
        ...
        // --- Main App & Routing ---
        const App = () => {
            const initialRoute = window.__INITIAL_ROUTE__ || 'text';
            ...
            const activeTool = tools.find(x => x.id === route) || tools[0];
                                ...
                                <div key={cat}>
                                    ...
                                    <div className="space-y-1">
                                        {tools.filter(x => x.cat === cat).map(item => (
                                            <a
                                                key={item.id}
                                                href={(routePaths[item.id] || `/#${item.id}`)}
                                                onClick={() => setMenuOpen(false)}
                                                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${route === item.id ? 'bg-slate-800 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'}`}
                                            >
                                                <Icon name={item.icon} size={18} className={route === item.id ? 'text-blue-300' : 'text-slate-400'} />
                                                {t.tools[item.id]}
                                            </a>
                                        ))}
                                    </div>
